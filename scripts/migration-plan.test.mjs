...
test("the auth schema ships outside the globbed directory", () => {
  ...
  const topLevelMigrations = readdirSync(migrationsDir);
  assert.equal(topLevelMigrations.includes(AUTH_MIGRATION), false);
  assert.equal(topLevelMigrations.includes("0002_benchproof.sql"), true);
