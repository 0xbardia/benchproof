...
test("emits complete share metadata from the app identity", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "benchproof.example.com",
    site: {
      title: "BenchProof — Prove the benchmark, not just the score",
      site_name: "BenchProof",
      description: "BenchProof verifies benchmark claims with evidence and GenLayer.",
      card: "custom",
      image: "/og.jpg",
    },
  });
  assert.match(out, /property="og:site_name" content="BenchProof"/);
  assert.match(
    out,
    /property="og:description" content="BenchProof verifies benchmark claims with evidence and GenLayer\."/,
  );
  assert.match(
    out,
    /name="twitter:title" content="BenchProof — Prove the benchmark, not just the score"/,
  );
  assert.match(
    out,
    /name="twitter:description" content="BenchProof verifies benchmark claims with evidence and GenLayer\."/,
  );
  assert.match(out, /name="twitter:image" content="https:\/\/benchproof\.example\.com\/og\.jpg"/);
});
...
test("site title BenchProof is a real name, not a sentinel", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
    site: { title: "BenchProof" },
  });
  assert.match(out, /property="og:title" content="BenchProof"/);
});
...
test("names the install page from host slug", () => {
  assert.equal(appNameFromHost("localhost:8080"), "BenchProof");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "BenchProof");
  assert.equal(appNameFromHost("wild-race.grok.me"), "Wild Race");
});

test("rejects hosts that are not plain slugs", () => {
  assert.equal(appNameFromHost("<script>alert(1)</script>"), "BenchProof");
  assert.equal(appNameFromHost('"><img src=x onerror=1>.grok.me'), "BenchProof");
});
