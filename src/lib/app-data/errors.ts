...
export type CallToolErrorKind =
  "pending" | "login" | "not_connected" | "scope_denied" | "access_denied" | "error";
...
export function classifyCallToolError(result: CallToolResult): CallToolErrorState | null {
  ...
  if (raw.includes("missing_connector_token")) {
    return {
      kind: "error",
      message: "Open this app from its host to load your data.",
      detail,
    };
  }
  if (isLoginRequired(result)) {
    return {
      kind: "login",
      message: "Continue with your account to load your data.",
      detail,
    };
  }
